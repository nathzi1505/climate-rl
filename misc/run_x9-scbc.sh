...
python -u "$BASE_DIR/rl-algos/$ALGO/main.py" --env_id "$ENV_ID" --optim_group "$TAG" --seed $SEED --wandb_group "$WANDB_GROUP" --no-track --total_timesteps 60000 --num_steps 200 --capture_video_freq 100
EOT
    done
    # sleep 30
done
