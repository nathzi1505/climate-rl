...
python -u "$BASE_DIR/rl-algos/$ALGO/main.py" --env_id "$ENV_ID" --optim_group "$TAG" --seed $SEED --wandb_group "$WANDB_GROUP" --total_timesteps 60000 --num_steps 200 --capture_video_freq 100 --actor_layer_size 64 --critic_layer_size 64 --no-track
EOT
    done
    # sleep 30
done
