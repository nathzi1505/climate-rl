...
class RadiativeConvectiveModelEnv(gym.Env):
            ...
            low=np.array(
                [
                    ...
                    *[
                        ...
                        for x in range(len(self.utils.Tobs.level) - 1)
                    ],
                ],
                dtype=np.float32,
            ),
    ...
    def step(self, action):
        emissivity, adj_lapse_rate, specific_humidity = (
            action[0],
            action[1 : 1 + len(self.utils.Tobs.level)],
            action[1 + len(self.utils.Tobs.level) :],
        )
    ...
    def reset(self, seed=None, options=None):
        ...
        h2o = climlab.radiation.ManabeWaterVapor(
            name="Water Vapour", state=rce_state, lev=self.utils.Tobs.level[1:]
        )
    ...
    def _render_frame(self):
        ...
        ax1_bars = ax1.bar(
            ...
            [
                params[0],
                np.mean(params[1 : 1 + len(self.utils.Tobs.level)]),
                1e3 * np.mean(params[1 + len(self.utils.Tobs.level) :]),
            ],
            color=ax1_colors,
            width=0.75,
        )
